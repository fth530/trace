...
const SLIDES = [
    {
        icon: 'wallet-outline',
        title: 'Harcamanı Gir',
        description: 'Ne kadar harcadığını anında yaz. Karmaşık menüler ve sıkıcı detaylarla uğraşma.',
        color: neonColors.cyan,
    },
    {
        icon: 'speedometer-outline',
        title: 'Limitini Koy',
        description: 'Günlük ve aylık bütçeni belirle. TRACE seni limite yaklaştığında uyarsın.',
        color: neonColors.pink,
    },
    {
        icon: 'planet-outline',
        title: 'Kontrolü Eline Al',
        description: 'Veriler cihazında kalır, analizler her şeyi özetler. Finansal özerkliğini kazan.',
    ...
    const handleNext = async () => {
        ...
        if (currentIndex < SLIDES.length - 1) {
            ...
            try {
                ...
                console.error('Failed to update onboarding setting', error);
            }
        }
    };
    ...
    return (
        <View className="flex-1 bg-zinc-950 items-center justify-between py-24">
            ...
            {currentIndex < SLIDES.length - 1 && (
                ...
                >
                    <Text className="text-zinc-500 font-medium">Atla</Text>
                </Pressable>
            )}
            ...
            <View className="w-full px-8 items-center mt-auto">
                <View className="flex-row gap-3 mb-10">
                    {SLIDES.map((_, index) => (
                        <View
                            key={index}
                            className={`h-2 rounded-full transition-all duration-300 ${index === currentIndex ? 'w-8 bg-white' : 'w-2 bg-white/20'
                                }`}
                            style={index === currentIndex ? neonShadow(currentSlide.color, 'sm') : undefined}
                        />
                    ))}
                </View>
